...
URLS = ['http://www.schaumburgmazda.com']
...
URLS.each do |url|
  ...
  while cars.empty?
    ...
    cars = full_doc.xpath('//*[@id="compareForm"]/div/div/ul/li')
    sleep(0.25)
  end
  ...
  PAGES.times do |n|
    cars.each do |car|
      ...
      unless worksheet.rows.map{|row| row[11]}.include?(vin)
        ...
        price = car.xpath(".//span[contains(@class, 'askingPrice')]/span[contains(@class, 'value')]").first.text.gsub(/\D/, "").to_i
